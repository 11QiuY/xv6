...
void mmap_test(void) {
  ...
  if (buf[0] != 'A') err("write to MAP_PRIVATE was written to file");
  ...
  for (i = 0; i < PGSIZE; i++) p[i] = 'B';
  for (i = PGSIZE; i < PGSIZE * 2; i++) p[i] = 'C';
  ...
  // written to the file.
  if ((fd = open(f, O_RDONLY)) == -1) err("open (4)");
  if (read(fd, buf, PGSIZE) != PGSIZE) err("dirty read #1");
  for (i = 0; i < PGSIZE; i++) {
    if (buf[i] != 'B') err("file page 0 does not contain modifications");
  }
  if (read(fd, buf, PGSIZE) != PGSIZE) err("dirty read #2");
  for (i = 0; i < PGSIZE; i++) {
    if (buf[i] != 'C') err("file page 1 does not contain modifications");
  }
  ...
  if ((fd = open(f, O_RDWR)) == -1) err("open");
  if (write(fd, "m", 1) != 1) err("write");
  close(fd);

  if (*p != 'm') err("read was not lazy");
...
void more_test() {
  ...
  *(p + PGSIZE) = 'Y';
  ...
  if (pid == 0) {
    *p = 'a';
    *(p + PGSIZE) = 'b';
    if (munmap(p + PGSIZE, PGSIZE) == -1) err("munmap");
  ...
  int st = 0;
  wait(&st);
  if (st != -1) err("child #1 read unmapped memory");
  ...
  if (pid == 0) {
    *p = 'c';
    *(p + PGSIZE) = 'd';
    if (munmap(p, PGSIZE) == -1) err("munmap");
    // this should cause a fatal fault
    printf("*p = %x\n", *p);
    exit(0);
  }
  st = 0;
  wait(&st);
  if (st != -1) err("child #2 read unmapped memory");
  ...
  *(p + PGSIZE) = 'R';
  ...
  if (fd < 0) err("open");
  int r = 0;
  if ((r = read(fd, buf, PGSIZE)) != PGSIZE) {
    printf("read %d\n", r);
    err("read");
  }
  ...
  st = 0;
  wait(&st);
  if (st != -1) err("child wrote read-only mapping");
