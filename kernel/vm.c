...
int supermappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa,
  ...
  uint64 a, last;
  pte_t *pte;
  // printf("supermappages: va=%ld size=%ld pa=%ld perm=%d\n", va, size, pa,
  // perm);

  if ((va % SUPERPGSIZE) != 0) panic("supermappages: va not aligned");

  if ((size % SUPERPGSIZE) != 0) panic("supermappages: size not aligned");

  if (size == 0) panic("supermappages: size");
  ...
  for (;;) {
    ...
    *pte = PA2PTE(pa) | perm | PTE_V | PTE_SP | PTE_R;
...
void breakpoint() { printf("breakpoint\n"); }
...
uint64 uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm) {
  ...
  for (a = oldsz; a < newsz; a += sz) {
    ...
    // printf("uvmalloc: va=%ld sz=%d pa=%ld pte=%ld\n", a, sz, (uint64)mem,
    // *(walk(pagetable, a, 0)));
  }
  return newsz;
}
...
uint64 superuvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz,
  ...
  for (a = oldsz; a < newsz; a += sz) {
    ...
    if (supermappages(pagetable, a, sz, (uint64)mem, PTE_R | PTE_U | xperm) !=
        0) {
      superfree(mem);
      uvmdealloc(pagetable, a, oldsz);
      return 0;
    }
    printf("superuvmalloc: va=%ld sz=%d\n pa=%ld pte=%ld\n", a, sz, (uint64)mem,
           *(walk(pagetable, a, 0)));
  }
  return newsz;
}
...
void tbprint(pagetable_t pagetable, int depth, int pre_va) {
  for (int i = 0; i < 512; i++) {
