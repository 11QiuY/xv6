...
void mmapunmap(pagetable_t pagetable, uint64 va, uint64 npages,
               struct mmap_VMA *v) {
  uint64 a;
  pte_t *pte;

  if ((va % PGSIZE) != 0) panic("mmapunmap: not aligned");
  ...
  for (a = va; a < va + npages * PGSIZE; a += PGSIZE) {
    ...
    if (PTE_FLAGS(*pte) == PTE_V) panic("uvmunmap: not a leaf");
    uint64 pa = PTE2PA(*pte);
    if (1 && (v->flags & MAP_SHARED)) {
      // write back to file
      struct file *f = v->file;
      begin_op();
      ilock(f->ip);
      int n = writei(f->ip, 0, (uint64)pa, a - v->addr + v->offset, PGSIZE);
      iunlock(f->ip);
      end_op();
      if (n < 0) {
        panic("mmapunmap(): filewrite failed\n");
        return;
      }
    }
    kfree((void *)pa);
